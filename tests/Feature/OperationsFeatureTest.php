...
class OperationsFeatureTest extends TestCase
{
    ...
    public function test_admin_can_open_every_operations_section(): void
    {
        ...
        foreach (['parents', 'classes', 'teachers', 'subjects', 'attendance', 'exams', 'results', 'announcements', 'events'] as $section) {
            $this->actingAs($admin)
                ->get(route('admin.operations', ['section' => $section]))
                ->assertOk();
        }
    }
    ...
    public function test_parent_can_be_created_linked_and_archived(): void
    {
        ...
        $this->assertDatabaseHas('students', ['id' => $student, 'parent_id' => $parent->id]);

        $this->actingAs($admin)->delete(route('admin.operations.destroy', $parent->id), ['section' => 'parents'])
            ->assertSessionHas('success');

        $this->assertNotNull(DB::table('parents')->where('id', $parent->id)->value('archived_at'));
        $this->actingAs($admin)->get(route('admin.operations', ['section' => 'parents']))->assertOk();
    ...
    public function test_teacher_and_subject_can_be_created_updated_and_archived(): void
    {
        ...
        $this->actingAs($admin)->post(route('admin.operations.store'), [
            ...
            'phone' => '0712345678',
            'employee_number' => 'TCH-010',
        ])->assertSessionHas('success');
        $teacher = DB::table('teachers')->where('employee_number', 'TCH-010')->first();

        $this->actingAs($admin)->put(route('admin.operations.update', $teacher->id), [
            'section' => 'teachers',
            'name' => 'Jane Wanjiku Updated',
        ...
        $this->actingAs($admin)->put(route('admin.operations.update', $subject->id), [
            'section' => 'subjects',
        ...
        $this->actingAs($admin)->delete(route('admin.operations.destroy', $subject->id), ['section' => 'subjects'])
            ->assertSessionHas('success');
    ...
    public function test_classes_validate_teacher_and_can_be_archived(): void
    {
        ...
        $this->actingAs($admin)->delete(route('admin.operations.destroy', $class->id), ['section' => 'classes'])
            ->assertSessionHas('success');
    ...
    public function test_assessment_and_announcement_workflows_preserve_status(): void
    {
        ...
        $this->actingAs($admin)->post(route('admin.operations.store'), [
            ...
            'body' => 'The parent meeting will be held on Friday.',
            'published' => 1,
        ])->assertSessionHas('success');
        $announcement = DB::table('announcements')->where('title', 'Parent Meeting')->first();
        $this->assertTrue((bool) $announcement->published);
        $this->assertNotNull($announcement->published_at);

        $this->actingAs($admin)->put(route('admin.operations.update', $announcement->id), [
            'section' => 'announcements',
            'title' => 'Parent Meeting Updated',
            'body' => 'Updated meeting notice.',
        ])->assertSessionHas('success');
        $announcement = DB::table('announcements')->find($announcement->id);
        $this->assertFalse((bool) $announcement->published);
        $this->assertNull($announcement->published_at);

        $this->actingAs($admin)->delete(route('admin.operations.destroy', $exam->id), ['section' => 'exams'])
            ->assertSessionHas('success');
    ...
    public function test_event_can_be_created_updated_and_archived(): void
    {
        ...
        $this->actingAs($admin)->put(route('admin.operations.update', $event->id), [
            'section' => 'events',
            'title' => 'Annual Sports Day',
            'event_date' => '2026-10-11',
            'location' => 'Main Field',
            'description' => 'Updated sports day notice.',
        ])->assertSessionHas('success');

        $this->assertDatabaseHas('events', ['id' => $event->id, 'title' => 'Annual Sports Day']);

        $this->actingAs($admin)->delete(route('admin.operations.destroy', $event->id), ['section' => 'events'])
            ->assertSessionHas('success');
        $this->assertNotNull(DB::table('events')->where('id', $event->id)->value('archived_at'));
    }

    public function test_archived_related_records_are_not_offered_or_displayed_as_active_context(): void
    {
        $admin = $this->admin();
        $teacher = DB::table('teachers')->insertGetId([
            'name' => 'Archived Teacher',
            'employee_number' => 'TCH-ARCH',
            'created_at' => now(),
            'updated_at' => now(),
            'archived_at' => now(),
        ]);
        ...
        $this->actingAs($admin)->get(route('admin.operations', ['section' => 'subjects']))
            ->assertOk()
            ->assertDontSee('Archived Subject');
        $this->actingAs($admin)->get(route('admin.operations', ['section' => 'teachers']))
            ->assertOk()
            ->assertDontSee('Archived Teacher');
        ...
        $this->assertDatabaseHas('subjects', ['id' => $subject, 'archived_at' => DB::table('subjects')->where('id', $subject)->value('archived_at')]);
    }
