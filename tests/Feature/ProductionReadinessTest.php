...
class ProductionReadinessTest extends TestCase
{
    ...
    {
        foreach (['/', '/about', '/academics', '/admissions', '/contact', '/login', '/register'] as $path) {
    ...
    public function test_authentication_redirects_users_to_their_role_area(): void
    {
        ...
        $this->post('/login', ['email' => $parent->email, 'password' => 'Password123!'])
    ...
    public function test_parent_payment_is_limited_to_linked_learners(): void
    {
        ...
        $response = $this->actingAs($parentUser)->post(route('portal.payments.pay'), [
    ...
    {
        $application = DB::table('admission_applications')->insertGetId([
            'student_name'=>'Approved Learner','date_of_birth'=>'2015-05-10','requested_class'=>'Grade 5',
            'parent_name'=>'Parent One','parent_phone'=>'+254712345678','parent_email'=>'parent@example.test',
            'status'=>'pending','created_at'=>now(),'updated_at'=>now(),
        ]);
        $admin = $this->user('admin');
        $payload = ['status'=>'approved'];

        $this->actingAs($admin)->patch(route('admin.admissions.status',$application), $payload)->assertSessionHas('success');
        $this->actingAs($admin)->patch(route('admin.admissions.status',$application), $payload)->assertSessionHas('success');

        $base = strtoupper(substr('ApprovedLearner',0,4));
        $this->assertSame(1, DB::table('students')->where('admission_number',$base.'-'.date('Y').'-'.str_pad((string)$application,4,'0',STR_PAD_LEFT))->count());
    }
    ...
    public function test_student_crud_rejects_duplicate_admission_numbers(): void
    {
        ...
        $this->actingAs($admin)->post(route('admin.students.store'), [
            'admission_number'=>'S-001','name'=>'Duplicate Learner','class_name'=>'Grade 5','fee_balance'=>0,
    ...
    public function test_attendance_upsert_preserves_original_creation_timestamp(): void
    {
        ...
        $this->actingAs($admin)->post(route('admin.operations.attendance'), [
            'student_id'=>$student,'attendance_date'=>'2026-09-10','status'=>'late','notes'=>'Arrived late',
        ])->assertSessionHas('success');

        $row = DB::table('attendance')->where('student_id',$student)->first();
        $this->assertSame('late',$row->status);
        $this->assertSame($old->format('Y-m-d H:i:s'), date('Y-m-d H:i:s', strtotime($row->created_at)));
    }
    ...
    public function test_missed_result_can_be_recorded_and_is_not_treated_as_zero(): void
    {
        ...
        $this->actingAs($admin)->post(route('admin.operations.results'), [
            'exam_id'=>$exam,'student_id'=>$student,'subject_id'=>$subject,'assessment_status'=>'missed','remarks'=>'Absent',
        ])->assertSessionHas('success');

        $result = DB::table('results')->where(['exam_id'=>$exam,'student_id'=>$student,'subject_id'=>$subject])->first();
        $this->assertNull($result->marks);
        $this->assertSame('MISSED', $result->achievement_level);
        $this->assertNull($result->achievement_points);
    }
    ...
    public function test_migrations_allow_nullable_result_marks(): void
    {
        ...
        if (Schema::getConnection()->getDriverName() === 'sqlite') {
            ...
            $this->assertSame(0, (int)$marks->notnull);
        }
    }
    ...
    public function test_mpesa_callback_rejects_amount_tampering(): void
    {
        ...
        $payment = DB::table('payments')->insertGetId([
            'student_id'=>$student,'parent_phone'=>'254712345678','payment_type'=>'school_fees','channel'=>'mpesa_stk',
            'amount'=>2000,'account_reference'=>'FEE-1','checkout_request_id'=>'ws_CO_123','status'=>'pending','verification_status'=>'pending',
            'created_at'=>now(),'updated_at'=>now(),
        ]);

        $response = $this->postJson('/api/mpesa/callback', ['Body'=>['stkCallback'=>[
            'CheckoutRequestID'=>'ws_CO_123','ResultCode'=>0,'CallbackMetadata'=>['Item'=>[
                ['Name'=>'MpesaReceiptNumber','Value'=>'ABC123'],['Name'=>'Amount','Value'=>1500],['Name'=>'PhoneNumber','Value'=>254712345678],
            ]],
        ]]]);
    ...
    public function test_mpesa_callback_is_idempotent_and_updates_balance_once(): void
    {
        ...
        $payment = DB::table('payments')->insertGetId([
            'student_id'=>$student,'parent_phone'=>'254712345678','payment_type'=>'school_fees','channel'=>'mpesa_stk',
            'amount'=>2000,'account_reference'=>'FEE-1','checkout_request_id'=>'ws_CO_456','status'=>'pending','verification_status'=>'pending',
            'created_at'=>now(),'updated_at'=>now(),
        ]);
        $callback = ['Body'=>['stkCallback'=>[
            'CheckoutRequestID'=>'ws_CO_456','ResultCode'=>0,'CallbackMetadata'=>['Item'=>[
                ['Name'=>'MpesaReceiptNumber','Value'=>'ABC456'],['Name'=>'Amount','Value'=>2000],['Name'=>'PhoneNumber','Value'=>254712345678],
            ]],
        ]]];
    ...
    public function test_duplicate_mpesa_receipt_is_rejected(): void
    {
        ...
        $this->postJson('/api/mpesa/callback',['Body'=>['stkCallback'=>['CheckoutRequestID'=>'ws_CO_789','ResultCode'=>0,'CallbackMetadata'=>['Item'=>[
            ['Name'=>'MpesaReceiptNumber','Value'=>'DUP123'],['Name'=>'Amount','Value'=>1000],
        ]]]]])->assertOk();
    ...
    public function test_admin_manual_completed_payment_updates_balance_and_creates_audit(): void
    {
        ...
        $this->actingAs($admin)->post(route('admin.payments.store'),[
            'student_id'=>$student,'parent_phone'=>'+254712345678','payment_type'=>'school_fees','amount'=>1000,
            'account_reference'=>'MANUAL-001','mpesa_receipt'=>'MAN123','status'=>'completed',
    ...
    public function test_cbc_service_does_not_send_duplicate_notifications_for_unchanged_report(): void
    {
        ...
        $subject = DB::table('subjects')->insertGetId(['name'=>'Mathematics','code'=>'MAT','created_at'=>now(),'updated_at'=>now()]);
        DB::table('results')->insert(['exam_id'=>$exam,'student_id'=>$student,'subject_id'=>$subject,'marks'=>80,'assessment_status'=>'present','grade'=>'EE2','achievement_level'=>'EE2','achievement_points'=>7,'created_at'=>now(),'updated_at'=>now()]);
        DB::table('portal_profiles')->insert(['user_id'=>$this->user('parent')->id,'portal_type'=>'parent','admission_number'=>'S-001','active'=>true,'created_at'=>now(),'updated_at'=>now()]);

        $service = app(CbcReportCardService::class);
        $first = $service->generateAndNotify($student,$exam);
        $second = $service->generateAndNotify($student,$exam);
