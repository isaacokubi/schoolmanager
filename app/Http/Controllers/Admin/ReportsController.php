...
class ReportsController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (in_array($report, ['overview', 'fees'], true)) {
            ...
            $data['payments'] = $query->latest('payments.id')->limit(100)->get();
        ...
        if (in_array($report, ['overview', 'attendance'], true)) {
            $data['attendance'] = DB::table('attendance')->leftJoin('students', 'students.id', '=', 'attendance.student_id')
                ->select('attendance.attendance_date', 'attendance.status', 'students.name as student_name', 'students.admission_number')
                ->orderByDesc('attendance.attendance_date')->orderBy('students.name')->limit(200)->get();
        ...
        if (in_array($report, ['overview', 'results'], true)) {
            $data['results'] = DB::table('results')->leftJoin('students', 'students.id', '=', 'results.student_id')->leftJoin('subjects', 'subjects.id', '=', 'results.subject_id')->leftJoin('exams', 'exams.id', '=', 'results.exam_id')
                ->select('results.marks', 'results.grade', 'students.name as student_name', 'students.admission_number', 'subjects.name as subject_name', 'exams.name as exam_name')
                ->orderByDesc('results.id')->limit(200)->get();
            $data['resultCount'] = DB::table('results')->count();
            $data['averageMarks'] = (float) (DB::table('results')->avg('marks') ?: 0);
        }
    ...
    private function studentBalanceTotal()
    {
        try {
            foreach (['fee_balance', 'balance', 'outstanding_balance'] as $column) {
                if (in_array($column, DB::getSchemaBuilder()->getColumnListing('students'), true)) return (float) DB::table('students')->sum($column);
            }
        } catch (\Throwable $e) {}
        return 0;
    }
