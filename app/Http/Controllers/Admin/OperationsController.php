...
class OperationsController extends Controller
{
    ...
    private function listing($section, Request $request)
    {
        ...
        if ($section === 'attendance') {
            $query = DB::table('attendance')->leftJoin('students','students.id','=','attendance.student_id')
            ...
            $query = DB::table('results')->leftJoin('students','students.id','=','results.student_id')
                ->leftJoin('exams','exams.id','=','results.exam_id')->leftJoin('subjects','subjects.id','=','results.subject_id')
        ...
        if ($search !== '') {
            ...
            elseif ($section === 'results') $query->where(function ($q) use ($search) { $q->where('students.name','like','%'.$search.'%')->orWhere('students.admission_number','like','%'.$search.'%')->orWhere('exams.name','like','%'.$search.'%')->orWhere('subjects.name','like','%'.$search.'%')->orWhere('results.achievement_level','like','%'.$search.'%')->orWhere('results.assessment_status','like','%'.$search.'%'); });
                ...
                $searchable = array_values(array_filter($columns, function ($column) { return !in_array($column, ['id','created_at','updated_at'], true); }));
                if ($searchable) $query->where(function ($q) use ($searchable, $search) { foreach ($searchable as $column) $q->orWhere($column, 'like', '%'.$search.'%'); });
            }
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        catch (\Throwable $e) { return back()->withErrors(['record'=>'The record could not be updated. Check for duplicate values or related records.'])->withInput(); }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        abort_unless(isset($this->tables[$section]), 404);
        abort_unless(DB::table($this->tables[$section])->where('id',$id)->exists(), 404);
        try { DB::table($this->tables[$section])->where('id',$id)->delete(); }
        catch (\Throwable $e) { return back()->withErrors(['record'=>'This record cannot be deleted because other records depend on it. Remove those related records first.']); }
    ...
    public function result(Request $request, CbcReportCardService $reportCards)
    {
        ...
        $level = $reportCards->level($data['marks'] === null ? null : (float)$data['marks']);
        $data['grade'] = $level['code'] === 'MISSED' ? null : $level['code'];
        $data['achievement_level'] = $level['code'] === 'MISSED' ? null : $level['code'];
        ...
        try {
            $reportCards->generateAndNotify((int)$data['student_id'], (int)$data['exam_id']);
        } catch (\Throwable $e) {
            report($e);
        }

        $message = $data['assessment_status'] === 'missed'
            ? 'Missed assessment recorded. The CBC report will include the missed result once the assessment set is complete.'
            : 'CBC result saved. The report card will be generated and notified automatically when all recorded subjects for this assessment are complete.';
    ...
    private function rules($section, $id=null)
    {
        $rules = [
            ...
            'results'=>[
                'exam_id'=>'required|exists:exams,id','student_id'=>'required|exists:students,id','subject_id'=>'required|exists:subjects,id',
                'assessment_status'=>'required|in:present,missed','marks'=>'nullable|required_if:assessment_status,present|numeric|min:0|max:100',
        ...
        if ($section === 'teachers') $rules[$section]['employee_number'][] = Rule::unique('teachers','employee_number')->ignore($id);
        if ($section === 'subjects') $rules[$section]['code'][] = Rule::unique('subjects','code')->ignore($id);
