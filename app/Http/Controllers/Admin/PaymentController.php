    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function($q) use ($search){
                $q->where('students.name','like',"%{$search}%")->orWhere('students.admission_number','like',"%{$search}%")->orWhere('payments.parent_phone','like',"%{$search}%")->orWhere('payments.mpesa_receipt','like',"%{$search}%");
            });
        }
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function() use ($data){
            ...
            if (!empty($data['student_id']) && $data['status']==='completed') {
                DB::table('students')->where('id',$data['student_id'])->decrement('fee_balance',(float)$data['amount']);
