                                    ...
                                    <div class="achievement-grid">
                                        @foreach($examResults as $result)
                                            <article class="achievement-item">
                                                <div><strong>{{ $result->subject_name }}</strong><small>{{ $result->subject_code ?: 'CBC learning area' }}</small></div>
                                                <strong class="mark">{{ $result->marks===null?'—':number_format((float)$result->marks,1).'/100' }}</strong>
...
@media(max-width:620px){.portal-main{width:94%;padding-top:18px}.portal-header{position:static;padding:12px 3%;flex-direction:column}.portal-actions{max-width:none;width:100%;justify-content:flex-start}.portal-actions .btn{flex:1}.hero,.card,.profile-card,.quick-actions,.payment-cta{padding:18px}.stats,.action-grid,.learner-details,.details{grid-template-columns:1fr}.stat-card{min-height:110px}.payment-cta{align-items:flex-start;flex-direction:column}.exam-heading{flex-direction:column}.exam-actions{width:100%}.exam-actions .btn{flex:1}.table-wrap{margin-left:-4px;margin-right:-4px}.learner-top{flex-direction:column}.brand-copy strong{font-size:14px}}
</style>
@endsection
