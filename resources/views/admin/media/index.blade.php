...
<div class="media-page">
    ...
    @if(session('success'))
        <div class="media-alert success" role="status">
            <div class="alert-icon">✓</div>
            <div><strong>Saved successfully</strong><span>{{ session('success') }}</span></div>
        </div>
    @endif

    @if($errors->any())
        <div class="media-alert danger" role="alert">
            <div class="alert-icon">!</div>
            <div><strong>Please review the upload</strong><span>{{ $errors->first() }}</span></div>
        </div>
    @endif
    ...
    <section class="media-upload-card">
        <div class="section-heading">
            <div>
                <span class="media-eyebrow">Add content</span>
                <h2>Upload a school photo or video</h2>
                <p>Use clear, school-appropriate media for classrooms, CBC learning, sports, events, facilities and community activities.</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.media.store') }}" enctype="multipart/form-data" class="media-form" id="media-upload-form">
            ...
            <div class="media-form-grid">
                ...
                <label>
                    ...
                    <input type="text" name="title" maxlength="160" placeholder="e.g. Grade 6 science practical">
                </label>

                <label class="full">
                    <span>Caption <em>optional</em></span>
                    <textarea name="caption" rows="3" maxlength="1000" placeholder="Describe the activity, learning moment or school event..."></textarea>
                </label>
                ...
                <div class="full upload-zone" id="upload-zone" tabindex="0" role="button" aria-label="Choose a media file">
                    ...
                    <div class="upload-zone-copy">
                        ...
                        <span>or <u>browse from this computer</u></span>
                        <small id="file-help">Images: JPG, PNG, WebP, GIF and other server-supported image formats.</small>
                    </div>
                    <input type="file" name="media" id="media-file" accept="image/*" required>
                </div>

                <div class="full selected-file" id="selected-file" hidden>
                    <div class="selected-file-icon" id="selected-file-icon">IMG</div>
                    <div class="selected-file-copy">
                        <strong id="selected-file-name">No file selected</strong>
                        <span id="selected-file-meta"></span>
                    </div>
                    <button type="button" id="clear-file" aria-label="Remove selected file">×</button>
                </div>

                <label class="media-check full">
                    <input type="checkbox" name="published" value="1" checked>
                    <span><strong>Publish immediately</strong><small>Make this media available on the public school website after upload.</small></span>
                </label>
            </div>

            <div class="media-form-actions">
                <button class="media-button primary" type="submit" id="upload-button"><span>Upload to school website</span><b>→</b></button>
            </div>
        </form>
    </section>
    ...
    <section class="media-library-head">
        <div>
            <span class="media-eyebrow">Content library</span>
            <h2>School media library</h2>
            <p>Review, reorder and control which media appears on the homepage.</p>
        </div>
        <div class="library-summary">
            <span class="summary-dot"></span>
            <strong>{{ $media->total() }}</strong> {{ $media->total() === 1 ? 'item' : 'items' }}
        </div>
    </section>
        ...
        <div class="media-library">
            @foreach($media as $item)
                @php
                    ...
                    $sizeMb = $item->size ? number_format($item->size / 1048576, 1) . ' MB' : null;
                @endphp
                <article class="media-card">
                    <div class="media-preview {{ $item->type === 'video' ? 'is-video' : '' }}">
                        @if($item->type === 'image')
                            <img src="{{ $mediaUrl }}" alt="{{ $item->title ?: 'School photo' }}" loading="lazy">
                        @else
                    ...
                    <div class="media-card-body">
                        <div class="media-card-top">
                            <span class="media-type-icon {{ $item->type }}">{{ $item->type === 'image' ? 'IMG' : 'VID' }}</span>
                            <span class="media-status {{ $item->published ? 'published' : 'hidden' }}"><i></i>{{ $item->published ? 'Published' : 'Hidden' }}</span>
                        </div>
                        ...
                        <div class="media-meta">
                            @if($sizeMb)<span>◉ {{ $sizeMb }}</span>@endif
                        ...
                        <form method="POST" action="{{ route('admin.media.update', $item->id) }}" class="media-edit-form">
                            ...
                            <div class="media-edit-row">
                                <label><span>Display order</span><input type="number" name="sort_order" min="0" value="{{ $item->sort_order }}"></label>
                                <label class="media-check compact">
                        ...
                        <form method="POST" action="{{ route('admin.media.destroy', $item->id) }}" class="delete-form" onsubmit="return confirm('Remove this media from the school website? This cannot be undone.');">
                            @csrf
                            @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateAccept() {
        ...
        if (help) help.textContent = type.value === 'video'
            ? 'Videos: MP4, WebM and other formats supported by the server.'
            : 'Images: JPG, PNG, WebP, GIF and other supported image formats.';
    }
    ...
    function showFile(fileObject) {
        ...
        meta.textContent = formatBytes(fileObject.size) + ' · ' + (fileObject.type || 'Media file');
        icon.textContent = fileObject.type && fileObject.type.startsWith('video/') ? 'VID' : 'IMG';
    ...
    if (zone) {
        ['dragenter', 'dragover'].forEach(function (eventName) { zone.addEventListener(eventName, function (e) { e.preventDefault(); zone.classList.add('dragging'); }); });
        ['dragleave', 'drop'].forEach(function (eventName) { zone.addEventListener(eventName, function (e) { e.preventDefault(); zone.classList.remove('dragging'); }); });
        zone.addEventListener('drop', function (e) {
            const dropped = e.dataTransfer.files[0];
            if (!dropped) return;
            try { const transfer = new DataTransfer(); transfer.items.add(dropped); file.files = transfer.files; } catch (err) {}
            showFile(dropped);
        });
    }
    if (clear) clear.addEventListener('click', clearFile);
    if (form && button) form.addEventListener('submit', function () { button.disabled = true; button.style.opacity = '.7'; button.querySelector('span').textContent = 'Uploading…'; });
});
</script>
@endsection
