...
<div class="admin-shell">
    <div class="admin-top"><strong>School Manager</strong><span>{{ auth()->user()->name }}</span></div>
    <div class="admin-layout">
        <aside class="sidebar">
            ...
            <a class="active" href="{{ route('admin.settings') }}">Settings</a>
        </aside>
        <main class="admin-main">
            <h1>School Settings</h1>
            @if(session('success'))<div class="alert success">{{ session('success') }}</div>@endif
            @if($errors->any())<div class="errors"><ul>@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>@endif
            <form method="POST" action="{{ route('admin.settings.update') }}" class="card form-grid">
                @csrf @method('PUT')
                ...
                <div class="field" style="justify-content:end"><button class="btn" type="submit">Save settings</button></div>
            </form>
        </main>
    </div>
</div>
