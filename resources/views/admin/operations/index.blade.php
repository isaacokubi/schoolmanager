    ...
    <form method="POST" action="{{ route('admin.operations.attendance') }}" class="grid-form">
        ...
        <label>Student
            <select name="student_id" required><option value="">Select student</option>
                @foreach($students as $student)<option value="{{ $student->id }}">{{ $student->name }} — {{ $student->admission_number }}</option>@endforeach
            </select>
        </label>
        <label>Date<input type="date" name="attendance_date" value="{{ date('Y-m-d') }}" required></label>
        <label>Status
            <select name="status" required><option value="present">Present</option><option value="absent">Absent</option><option value="late">Late</option><option value="excused">Excused</option></select>
        </label>
    ...
    <form method="POST" action="{{ route('admin.operations.results') }}" class="grid-form">
        ...
        <label>Exam
            <select name="exam_id" required><option value="">Select exam</option>
                @foreach($exams as $exam)<option value="{{ $exam->id }}">{{ $exam->name }} — {{ $exam->term }} {{ $exam->academic_year }}</option>@endforeach
            </select>
        </label>
        <label>Student
            <select name="student_id" required><option value="">Select student</option>
        ...
        <label>Subject
            <select name="subject_id" required><option value="">Select subject</option>
...
<div class="card">
    ...
    <form method="GET" class="search">
        ...
        @if(request('search'))<a href="{{ route('admin.operations',['section'=>$section]) }}">Clear</a>@endif
    </form>
                            ...
                            <form method="POST" action="{{ route('admin.operations.update',$row->id) }}" class="grid-form">
                                ...
                                @if($section === 'attendance')
                                    <label>Student<select name="student_id" required>
                                        @foreach($students as $student)<option value="{{ $student->id }}" {{ $row->student_id == $student->id ? 'selected' : '' }}>{{ $student->name }} — {{ $student->admission_number }}</option>@endforeach
                                    </select></label>
                                    ...
                                    <label>Status<select name="status" required>
                                        @foreach(['present','absent','late','excused'] as $status)<option value="{{ $status }}" {{ $row->status === $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>@endforeach
                                    </select></label>
                                    <label>Notes<textarea name="notes">{{ $row->notes }}</textarea></label>

                                @elseif($section === 'results')
                                    <label>Exam<select name="exam_id" required>
                                        @foreach($exams as $exam)<option value="{{ $exam->id }}" {{ $row->exam_id == $exam->id ? 'selected' : '' }}>{{ $exam->name }} — {{ $exam->term }} {{ $exam->academic_year }}</option>@endforeach
                                    </select></label>
                                    <label>Student<select name="student_id" required>
                                        @foreach($students as $student)<option value="{{ $student->id }}" {{ $row->student_id == $student->id ? 'selected' : '' }}>{{ $student->name }} — {{ $student->admission_number }}</option>@endforeach
                                    </select></label>
                                    <label>Subject<select name="subject_id" required>
                                        @foreach($subjects as $subject)<option value="{{ $subject->id }}" {{ $row->subject_id == $subject->id ? 'selected' : '' }}>{{ $subject->name }}</option>@endforeach
                                    </select></label>
                                    ...
                                    @foreach((array)$row as $k => $v)
                                        @if(!in_array($k,['id','created_at','updated_at','published_at','grade','teacher_id'],true))
                                            ...
                                            @elseif(in_array($k,['body','description','notes'],true))
                                                <label>{{ ucwords(str_replace('_',' ',$k)) }}<textarea name="{{ $k }}">{{ $v }}</textarea></label>
                                            @else
                                                @php
                                                    $inputType = 'text';
                                                    if ($k === 'email') $inputType = 'email';
                                                    elseif ($k === 'academic_year') $inputType = 'number';
                                                    elseif (in_array($k,['start_date','end_date','event_date'],true)) $inputType = 'date';
                                                @endphp
                                                <label>{{ ucwords(str_replace('_',' ',$k)) }}<input type="{{ $inputType }}" name="{{ $k }}" value="{{ $v }}"></label>
                                            @endif
                                        @endif
                                    @endforeach
                                    @if($section === 'subjects')
                                        <label>Teacher<select name="teacher_id"><option value="">Unassigned</option>
                                            @foreach($teachers as $teacher)<option value="{{ $teacher->id }}" {{ ($row->teacher_id ?? null) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }}</option>@endforeach
                                        </select></label>
                                    @endif
                                @endif
