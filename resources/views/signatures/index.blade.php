...
<div class="card" style="max-width:760px;margin:24px auto;padding:24px">
    ...
    @if($signaturePath)
        <div style="margin:18px 0;padding:14px;background:#f8fafc;border:1px solid #d9e2eb;border-radius:8px"><strong>Current signature</strong><br><img src="{{ route('media.file', ['path' => ltrim($signaturePath, '/')]) }}" alt="Current signature" style="display:block;max-width:320px;max-height:110px;margin-top:10px;object-fit:contain;background:#fff;border:1px solid #e2e8f0;padding:8px"></div>
    @else
